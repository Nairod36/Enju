...
export function AppDashboard() {
  ...
  const levelUpUser = async (experienceGain: number) => {
    if (!address || !isAuthenticated) {
      console.warn("❌ Impossible d'augmenter l'expérience : utilisateur non connecté");
      return;
    }
    ...
    try {
      ...
      console.log(`🎮 Augmentation de l'expérience: +${experienceGain} XP pour ${address}`);
      ...
      if (response.ok) {
        ...
        console.error('❌ Erreur lors de la mise à jour de l\'expérience:', response.statusText);
      }
    } catch (error) {
      console.error('❌ Erreur lors de l\'appel d\'augmentation d\'expérience:', error);
    }
  };
  ...
  const saveIslandAfterTreePlant = async () => {
    if (!activeIsland || !islandRef.current) {
      console.warn("❌ Impossible de sauvegarder : île active ou référence manquante");
      return;
    }
    ...
    try {
      ...
      if (savedIsland) {
        console.log("✅ Île sauvegardée automatiquement après plantation d'arbre");
      ...
      {showWelcome && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <WelcomeNewUser 
            ...
            bridgeCount={bridgeHistory.filter((b) => b.status === "COMPLETED").length}
            memberSince={userIslandData?.createdAt
              ? new Date(userIslandData.createdAt).toLocaleDateString("en-US", {
                  month: "short",
                  year: "numeric",
                })
              : undefined
