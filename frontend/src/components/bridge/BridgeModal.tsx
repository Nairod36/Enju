...
const stepConfigs = {
  'ethereum-to-near': [
    { id: 'initiate', label: 'Initiate ETH Bridge', description: 'Creating escrow transaction' },
    { id: 'confirm', label: 'Confirm Transaction', description: 'Waiting for ETH confirmation' },
    { id: 'create-htlc', label: 'Create NEAR HTLC', description: 'Setting up NEAR contract' },
    { id: 'complete', label: 'Complete Bridge', description: 'Finalizing cross-chain transfer' },
  ],
  'near-to-ethereum': [
    { id: 'initiate', label: 'Initiate NEAR Bridge', description: 'Creating NEAR HTLC' },
    { id: 'confirm', label: 'Confirm NEAR Tx', description: 'Waiting for NEAR confirmation' },
    { id: 'create-escrow', label: 'Create ETH Escrow', description: 'Setting up ETH contract' },
    { id: 'complete', label: 'Complete Bridge', description: 'Finalizing cross-chain transfer' },
  ],
};
...
export function BridgeModal({ isOpen, onClose, bridgeData }: BridgeModalProps) {
  ...
  const direction = bridgeData ? `${bridgeData.fromChain}-to-${bridgeData.toChain}` as keyof typeof stepConfigs : 'ethereum-to-near';
  ...
  const startBridgeProcess = async () => {
    ...
    try {
      ...
      addLog(`🚀 Initiating ${bridgeData.fromChain.toUpperCase()} → ${bridgeData.toChain.toUpperCase()} bridge...`, 'info');
      addLog(`💰 Amount: ${bridgeData.fromAmount} ${bridgeData.fromChain.toUpperCase()}`, 'info');
      ...
      await new Promise(resolve => setTimeout(resolve, 2000));
      const mockBridgeId = `bridge_${Math.random().toString(16).substring(2, 10)}`;
      ...
      addLog(`✅ ${bridgeData.fromAmount} ${bridgeData.fromChain.toUpperCase()} → ${bridgeData.toChain.toUpperCase()} transfer complete`, 'success');
  ...
  const getStepIcon = (stepIndex: number) => {
    if (stepIndex < currentStep) return <CheckCircle className="w-5 h-5 text-green-500" />;
    if (stepIndex === currentStep && status === 'pending') return <Clock className="w-5 h-5 text-blue-500 animate-pulse" />;
    if (status === 'error' && stepIndex === currentStep) return <AlertCircle className="w-5 h-5 text-red-500" />;
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden">
        ...
        <div className="space-y-6">
              ...
              {steps.map((step, index) => (
                <div key={step.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-center">
                      <span className={`font-medium ${index <= currentStep ? 'text-gray-900' : 'text-gray-500'}`}>
                        {step.label}
                      </span>
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="ml-2 font-medium">{bridgeData.fromAmount} {bridgeData.fromChain.toUpperCase()}</span>
                </div>
                <div>
                  <span className="text-gray-500">To:</span>
                  <span className="ml-2 font-medium">{bridgeData.fromAmount} {bridgeData.toChain.toUpperCase()}</span>
                </div>
                {bridgeData.nearAccount && (
                  <div className="col-span-2">
                    <span className="text-gray-500">NEAR Account:</span>
                    <span className="ml-2 font-medium">{bridgeData.nearAccount}</span>
                  </div>
                )}
                {secret && (
                  <div className="col-span-2">
          ...
          {/* Logs Section */}
          <div className="space-y-3">
            <h4 className="font-medium text-gray-900">Bridge Logs</h4>
            ...
            <div className="flex gap-2">
              {bridgeId && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => window.open(`${BRIDGE_CONFIG.listenerApi}/bridges`, '_blank')}
                >
                  View Bridge
